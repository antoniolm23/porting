...
static struct nm_desc *net_cap = 0;
	...
	const NETSTAT *ns;
{
	...
	if (ph->pcap) {
        ...
        */
		...
		net_cap = nm_open(strcpy(name, ph->name), NULL, 0, 0);
		if (buf[0] != '\0')
			screen_status("%s: %s", ph->name, buf);
		if(!net_cap) return -1;
		//END MODIFIED PART
	...
	if (!cisco_netflow_dump) {
		//opens a file and return the writer to the file
...
live_pcap_dump()
{
	...
	while (live_pcap && live_dump) {
		op = pcap_dispatch(live_pcap, -1, live_pcap_parse,
				   (u_char *)live_dump);
	*/
	while(nm_desc && live_dump) {
		op = nm_dispatch(nm_desc, -1, live_pcap_parse, (u_char*)live_dump);
	...
	const u_char *p;
{
	...
	if(parse_dk(&ns, pcap_datalink(nm_desc), h->len, p) <0)
		return;
